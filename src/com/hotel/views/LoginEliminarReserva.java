...
public class LoginEliminarReserva extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LoginEliminarReserva frame = new LoginEliminarReserva();
	 ...
	 * Create the frame.
	 */
	public LoginEliminarReserva() {
		
		this.loginController = new LoginController();
		setIconImage(Toolkit.getDefaultToolkit().getImage(LoginEliminarReserva.class.getResource("/imagenes/aH-40px.png")));
		setResizable(false);
		setUndecorated(true);
		...
		lblImagen.setIcon(new ImageIcon(LoginEliminarReserva.class.getResource("/imagenes/login.png")));
	...
	private void Login() {
		...
		if (acceso > 3) {
			EliminarReservas eliminar = new EliminarReservas();
			eliminar.setVisible(true);
			dispose();
		} else if (acceso > 0 && acceso <4) {
			JOptionPane.showMessageDialog(this, "El usuario " + txtUsuario.getText() + " no puede eliminar datos");
			txtUsuario.setText("Ingrese su nombre de usuario");
			txtUsuario.setForeground(Color.gray);
			txtContrasena.setText("********");
			txtContrasena.setForeground(Color.gray);
